...
cursor = db.cursor()
querySetup="SELECT DISTINCT ClusterName FROM GNPNDB.EngineeredGeneClusters WHERE EngineeredGeneClusters.GeneClusterID >25"
start=db.cursor()
...
while names:
        nameList=names
        print nameList[0]
        queryStart = (" SELECT Sequence FROM GNPNDB.NewPlasmidID Where GeneClusterID = (Select GeneClusterID from GNPNDB.EngineeredGeneClusters Where ClusterName= '"+ nameList[0]+ "' and Version=2.2 )")
        weird=db.cursor()
                ...
                if row!=None:
                        if(plasmidId!=row[4]):        
                                ...
                                recordTwo= SeqRecord(trueFullSeq,id='123456789',  # random accession number
                                        name=nameList[0],description='An example GenBank file generated by GPNPN LADY')
